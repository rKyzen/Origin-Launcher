...
class DeviceProfileOverrides @Inject constructor(
    ...
    fun getTextFactors(): TextFactors = TextFactors(preferenceManager2)
    ...
    data class Options(
        ...
        fun applyUi(idp: InvariantDeviceProfile, context: Context) {
            val originConfig = OriginModeManager.getInstance(context).currentConfig()
            ...
            val wsDensity = originConfig.workspaceDensity
            idp.iconSize[INDEX_DEFAULT] *= iconSizeFactor * wsDensity
            idp.iconSize[INDEX_LANDSCAPE] *= iconSizeFactor * wsDensity
            idp.iconSize[INDEX_TWO_PANEL_PORTRAIT] *= iconSizeFactor * wsDensity
            idp.iconSize[INDEX_TWO_PANEL_LANDSCAPE] *= iconSizeFactor * wsDensity

            val adDensity = originConfig.appDrawerDensity
            idp.allAppsIconSize[INDEX_DEFAULT] *= allAppsIconSizeFactor * adDensity
            idp.allAppsIconSize[INDEX_LANDSCAPE] *= allAppsIconSizeFactor * adDensity
            idp.allAppsIconSize[INDEX_TWO_PANEL_PORTRAIT] *= allAppsIconSizeFactor * adDensity
            idp.allAppsIconSize[INDEX_TWO_PANEL_LANDSCAPE] *= allAppsIconSizeFactor * adDensity
