...
class AllAppsSearchInput(context: Context, attrs: AttributeSet?) :
    ...
    private val originConfig = OriginModeManager.getInstance(context).currentConfig()
    private val supportBlur = BlurUtils.supportsBlursOnWindows()
    private val bg = if (originConfig.isFloatingSearchBar) {
        val config = originConfig
        val density = context.resources.displayMetrics.density
        val cornerRadius = config.cornerRadiusDp * density
        val glassAlpha = (config.glassOpacity * 255).toInt()
        android.graphics.drawable.PaintDrawable(android.graphics.Color.argb(glassAlpha, 255, 255, 255)).apply {
    ...
    override fun onFinishInflate() {
        ...
        input.onFocusChangeListener = OnFocusChangeListener { _, hasFocus ->
            ...
            if (::appsView.isInitialized) {
                appsView.searchRecyclerView.invalidate()
            }
        }
    ...
    private fun notifyResultChanged() {
        appsView.searchRecyclerView.onSearchResultsChanged()
    }
