...
class LawnchairIconProvider @Inject constructor(
    ...
    private val originModeManager = OriginModeManager.getInstance(context)
    private val themedIconsEnabled get() = prefs.themedIcons.get() || originModeManager.currentConfig().useMonochromeIcons
    ...
    override fun getIcon(
            ...
            when {
                ...
                packageName == mCalendar.packageName -> {
                    ...
                    themedIcon = if (themeData != null) {
                        val bg = if (isMono) ColorDrawable(Color.TRANSPARENT) else themedColors[0].toDrawable()
                        val fgTint = if (isMono) Color.WHITE else themedColors[1]
                        CustomAdaptiveIconDrawable(
                            bg,
                            themeData.loadPaddedDrawable().apply { setTint(fgTint) },
                        )
                    } else {
                        null
                    }
        ...
        var result = themedIcon ?: iconPackIcon ?: super.getIcon(info, appInfo, iconDpi)

        if (prefs.forceIconMonochrome.get() && result is AdaptiveIconDrawable) {
            try {
                result.mutate()
                val mono = result.monochrome
                if (mono != null) {
                    mono.setTint(Color.WHITE)
                    return AdaptiveIconDrawable(ColorDrawable(Color.TRANSPARENT), mono)
                }
            } catch (e: Exception) {
                Log.e(TAG, "Failed to create monochrome icon", e)
            }
        }
    ...
    override fun getStateForApp(info: ApplicationInfo?): String {
        ...
        return "$base|lc:" +
            ...
            "om=${originModeManager.currentConfig().useMonochromeIcons}"
    }
