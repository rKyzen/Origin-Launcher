...
class PreferenceManager2 @Inject constructor(
    ...
    val originMode = preference(
        key = stringPreferencesKey(name = "origin_mode"),
        defaultValue = OriginMode.Pure,
        parse = OriginMode::fromString,
        save = { it.storageName },
    ...
    val hotseatQsbProvider = preference(
        ...
        save = { provider: QsbSearchProvider -> provider.id },
        onSet = { reloadHelper.recreate() },
    )
    ...
    val dockGlassTransparency = preference(
        key = floatPreferencesKey("dock_glass_transparency"),
        defaultValue = 0.12f,
    )

    val dockBlurRadius = preference(
        key = intPreferencesKey("dock_blur_radius"),
        defaultValue = 0,
    )

    val dockAccentColor = preference(
        key = booleanPreferencesKey("dock_accent_color"),
        defaultValue = false,
    )

    val firstRunCompleted = preference(
        key = booleanPreferencesKey("first_run_completed"),
        defaultValue = false,
    )
