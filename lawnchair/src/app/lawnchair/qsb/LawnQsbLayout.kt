...
class LawnQsbLayout(context: Context, attrs: AttributeSet?) : FrameLayout(context, attrs) {
    ...
    private fun setUpBackground(themed: Boolean = false) {
        ...
        if (originConfig.isFloatingSearchBar) {
            val density = context.resources.displayMetrics.density
            val cornerRadius = originConfig.cornerRadiusDp * density
            val glassAlpha = (originConfig.glassOpacity * 255).toInt()

            val backgroundDrawable = PaintDrawable(Color.argb(glassAlpha, 255, 255, 255)).apply {
                setCornerRadius(cornerRadius)
            }
