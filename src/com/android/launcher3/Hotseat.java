...
import app.lawnchair.origin.OriginModeConfig;
...
public class Hotseat extends CellLayout implements Insettable {
    ...
    private void setUpBackground() {
        OriginModeConfig originConfig = OriginModeManager.getInstance(getContext()).currentConfig();

        if (originConfig.isFloatingDock()) {
            float density = getResources().getDisplayMetrics().density;
            int insetH = (int) (16 * density);
            int insetV = (int) (8 * density);
            int cornerRadius = (int) (originConfig.getCornerRadiusDp() * density);
            float userAlpha = PreferenceExtensionsKt.firstBlocking(preferenceManager2.getDockGlassTransparency());
            int alpha = (int) (userAlpha * 255);

            boolean useAccent = PreferenceExtensionsKt.firstBlocking(preferenceManager2.getDockAccentColor());
            int baseColor;
            if (useAccent) {
                var accent = PreferenceExtensionsKt.firstBlocking(preferenceManager2.getAccentColor());
                baseColor = accent.getColorPreferenceEntry().getLightColor().invoke(getContext());
            } else {
            ...
            InsetDrawable insetBg = new InsetDrawable(bg, insetH, insetV, insetH, insetV);
            setBackground(insetBg);
